...
const Page = () => {
  ...
  useEffect(() => {
    // 1. Request Plaid link_token from backend
  ...
  // 2. onSuccess callback when user links bank account
  ...
  const config = {
    ...
    const { open, ready } = usePlaidLink(linkToken ? config : {});
    ...
    <div style={{ width: "100%", height: "605px", position: "relative" }}>
      ...
      <div className="-mt-80 relative">
        ...
        <div className="absolute inset-0 flex justify-center items-center">
          <Image src="/satrun.png" alt="Saturn" width={700} height={700} />
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-6 items-center px-4 md:hidden -mt-140">
          ...
          <Card
            ...
            showProgress
          />
          <GetStarted open={open} ready={ready} />
        </div>
...
/* ---------- Get Started Button (Plaid Link) ---------- */
const GetStarted = ({ open, ready }) => (
  <button
    onClick={() => open()}
    disabled={!ready}
    type="button"
